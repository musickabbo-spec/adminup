...
def execute_user_task(message, secrets=None):
    """ইউজার মোডের সমস্ত সাধারণ কন্ডিশনাল কঙ্কাল (Ping কমান্ডসহ আপডেট করা হয়েছে)"""
...
def execute_admin_task(message, secrets):
    """অ্যাডমিন মোডের সমস্ত এআই ব্রেন, স্মার্ট প্রম্পট এবং আউটপুট ক্লিন-আপ মেকানিজম"""
...
- ping (🏓 Pong! K-Agent Core Engine responsive check)
...
3. Keep answers extremely short, smart, and token-optimized."""
    ...
    # 💾 এআই মেমোরি ইন্টিগ্রিটি: হিস্ট্রিতে মূল রেসপন্সটিই (কোডসহ) সংরক্ষণ করা হবে, যাতে এআই সব মনে রাখতে পারে
    ...
    # 🧹 স্ক্রিন ক্লিন-আপ লজিক: চ্যাট উইন্ডো থেকে কোড এবং লেজারের র ডাটা ফিল্টার করে বাদ দেওয়া
    def remove_block(text, start_tag, end_tag):
        if start_tag in text and end_tag in text:
            try:
                parts = text.split(start_tag)
                prefix = parts[0]
                suffix = parts[1].split(end_tag)[1]
                # ট্যাগ দুটি পাশাপাশি রেখে মাঝখানের ডাটা ডিলিট করা হচ্ছে
                return prefix + start_tag + end_tag + suffix
            except:
                pass
        return text

    # চ্যাট উইন্ডোতে বড় ডাটা হাইড করা
    chat_display_response = ai_response
    chat_display_response = remove_block(chat_display_response, "===NEW_CODE_START===", "===NEW_CODE_END===")
    chat_display_response = remove_block(chat_display_response, "===LEDGER_START===", "===LEDGER_END===")

    # ফ্রন্টএন্ড ট্যাগের সুন্দর রূপান্তর
    clean_response = chat_display_response.replace("===NEW_CODE_START===", "\n`[⚡ System Deploying New Code...]`\n").replace("===NEW_CODE_END===", "\n`[✅ Deploy Complete]`\n")
